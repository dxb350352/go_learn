...
func (f *File) Write(writer io.Writer) (err error) {
	...
	var fSheets = make([]*Sheet, len(f.Sheets))
	copy(fSheets, f.Sheets)
	for i, v := range fSheets {
		sheet := &Sheet{Name: v.Name, File: v.File}
		f.Sheets[i] = sheet
		f.Sheet[v.Name] = sheet
	}
	...
	for i, v := range fSheets {
		f.Sheets[i] = v
		f.Sheet[v.Name] = v
		parts, err = f.MarshallParts(refTable)
		if err != nil {
			return
		}
		partName := fmt.Sprintf("xl/worksheets/sheet%d.xml", i+1)
		fmt.Println(partName, len(parts[partName]), ".......")
		var writer io.Writer
		writer, err = zipWriter.Create(partName)
		if err != nil {
			return
		}
		_, err = writer.Write([]byte(parts[partName]))
		if err != nil {
			return
		}
		sheet := &Sheet{Name: v.Name, File: v.File}
	...
	for partName, part := range parts {
		if strings.HasPrefix(partName, "xl/worksheets/") {
			continue
		}
		var writer io.Writer
		writer, err = zipWriter.Create(partName)
...
func (f *File) MarshallParts(refTable *RefTable) (map[string]string, error) {
	var parts map[string]string
