...
func EncryptCbcPadding(key, iv, data []byte) []byte {
	c, err := NewCipher(key)
	if err != nil {
		return []byte{}
	}
	encrypter := cipher.NewCBCEncrypter(c, iv)
	paddingin := PKCS5Padding(data, BlockSize)
	result := make([]byte, len(paddingin))
	encrypter.CryptBlocks(result, paddingin)
	return result
}

func DecryptCbcPadding(key, iv, data []byte) []byte {
	c, err := NewCipher(key)
	if err != nil {
		return []byte{}
	}
	...
	decrypter.CryptBlocks(plain, data)
